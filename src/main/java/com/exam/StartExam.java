	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("ExamLoggedIn") != null && (boolean) session.getAttribute("ExamLoggedIn")) { 
			...
			try {
				...
				if(Long.compare(examEndTime, currentTime) > 0) { 
					...
					if(givenAttempts < numberOfAttempts) {
						if(attemptId == 0) { // attempt not given
							...
							if(currentTime >= startTime && currentTime <= (startTime + windowTime)) { // student will be allowed for attempt in window time
					...
					Attempt.endExam(studentId, examId);
